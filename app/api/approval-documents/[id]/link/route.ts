...
import { createLink, reissueLink, getActiveLinkToken } from '@/lib/approval-doc/token';
import { writeApprovalAuditLog } from '@/lib/approval-doc/audit';

/** app/api/supplier-requests/[id]/link/route.ts와 동일한 권한 정책: 생성/재발급은
...
async function assertLinkPermission(projectId: string) {
  ...
  const db = getDb();
  const project = db.prepare('SELECT created_by FROM approval_doc_projects WHERE id=?').get(projectId) as { created_by: string | null } | undefined;
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const db = getDb();
  const existing = db.prepare('SELECT id FROM approval_doc_links WHERE project_id=? AND is_active=1').get(id) as { id: string } | undefined;
  ...
  return NextResponse.json({ data: { url: `${baseUrl}/approval-form/${token}` } });
}
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (check.error) return check.error;
  const db = getDb();
  const link = db.prepare('SELECT created_at FROM approval_doc_links WHERE project_id=? AND is_active=1').get(id) as { created_at: string } | undefined;
  const token = link ? getActiveLinkToken(id) : null;
  const baseUrl = process.env.NEXT_PUBLIC_APP_URL || `${req.nextUrl.protocol}//${req.nextUrl.host}`;
  return NextResponse.json({
    data: { hasActiveLink: !!link, createdAt: link?.created_at ?? null, url: token ? `${baseUrl}/approval-form/${token}` : null },
  });
}
