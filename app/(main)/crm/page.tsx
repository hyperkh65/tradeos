...
import { triggerPrint, downloadFile } from '@/lib/tauri-print';
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-2">
      <div className="bg-background rounded-xl shadow-2xl w-full max-w-4xl max-h-[95vh] flex flex-col">
        ...
        <div className="p-4 border-t shrink-0 flex flex-wrap gap-2 justify-end">
          ...
          <Button type="button" variant="outline" disabled={!savedOnce} onClick={() => downloadFile(`/api/sales/${sale.id}/custom-statement/excel`)}>
            엑셀 다운로드
          </Button>
          <Button type="button" variant="outline" disabled={!savedOnce} onClick={() => downloadFile(`/api/sales/${sale.id}/custom-statement/pdf`)}>
            PDF 다운로드
          </Button>
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-2">
      <div className="bg-background rounded-xl shadow-2xl w-full max-w-5xl max-h-[95vh] flex flex-col">
        ...
        <div className="p-4 border-t shrink-0 flex flex-wrap gap-2 justify-end">
          <Button type="button" variant="outline" onClick={onClose}>닫기</Button>
          <Button type="button" variant="outline" disabled={!savedOnce} onClick={() => downloadFile(`/api/sales/${sale.id}/settlement-statement/excel`)}>
            엑셀 다운로드
          </Button>
          <Button type="button" variant="outline" disabled={!savedOnce} onClick={() => downloadFile(`/api/sales/${sale.id}/settlement-statement/pdf`)}>
            인쇄(PDF)
          </Button>
          <Button type="button" onClick={save} disabled={saving}>
            {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : '저장'}
