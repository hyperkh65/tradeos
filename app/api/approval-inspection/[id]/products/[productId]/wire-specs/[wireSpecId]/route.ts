...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string; productId: string; wireSpecId: string }> }) {
  const user = await getSessionUser();
  if (!user) return NextResponse.json({ error: '인증이 필요합니다' }, { status: 401 });
  const { id, productId, wireSpecId } = await params;
  const db = getDb();
  ...
  const before = db.prepare('SELECT * FROM approval_inspection_wire_specs WHERE id=? AND product_id=? AND project_id=?').get(wireSpecId, productId, id) as Record<string, unknown> | undefined;
  ...
  writeInspectionAuditLog({ projectId: id, action: 'wire_spec_update', actorType: 'internal', actorUserId: user.id, actorUserName: user.name, before, req });
