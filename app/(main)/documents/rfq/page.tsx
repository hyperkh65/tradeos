...
import {
  ...
  MapPin, Phone, Mail, Search, FilePlus2,
} from 'lucide-react';
...
const emptyItem = (): RfqItem => ({ name: '', specification: '', qty: 0, unit: 'EA', remark: '' });
const emptyData = (): RfqData => ({
  date: todayStr(), validUntil: addDays(30),
...
export default function RfqPage() {
  ...
  const openDoc = (d: DocRow) => { setSelected(d); setData({ ...emptyData(), ...d.data }); setListOpen(false); };
              ...
              <div key={i} className="border border-border rounded-lg overflow-hidden">
                ...
                <div className="grid grid-cols-6 gap-3 p-3">
                  <div className="col-span-2">
                    <label className="text-[10px] text-muted-foreground">품목명</label>
                    <input value={it.name} onChange={e => updateItem(i, { name: e.target.value })} placeholder="예: LED 모듈 25W 주광색"
