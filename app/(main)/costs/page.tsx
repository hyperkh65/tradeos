...
function CostModal({ record, onClose, onSave }: {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const finalCostAmount = effectiveCostItems ? costItemsTotal : (isDetailType && lineItems.length > 0 ? lineItemsTotal : parseFloat(form.costAmount || '0'));
      const body = {
        ...
        billAmount: isDetailType && lineItems.length > 0 ? lineItemsTotal : (form.billAmount ? parseFloat(form.billAmount) : undefined),
        ...
        lineItems: isDetailType ? lineItems : [],
      ...
      onSave();
      // 신규 저장 후 파일이 있으면 업로드 안내
      if (isNew && savedId) {
        window.open(`/costs/expense-print?id=${savedId}`, '_blank');
      }
    } catch (e) { alert(`오류: ${e}`); }
            ...
            <div className="space-y-3">
              ...
              <div className="bg-orange-50 border border-orange-200 rounded-lg p-3 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    {lineItems.length > 0 && (
                      <div className="text-xs font-bold text-orange-800">청구 합계: {lineItemsTotal.toLocaleString()} {form.billCurrency}</div>
                    )}
                    {costItems.length > 0 && (
                      <button type="button"
                        onClick={() => setLineItems(costItems.map(i => ({ ...i })))}
                        className="text-[10px] border border-orange-300 text-orange-700 px-2 py-0.5 rounded hover:bg-orange-100">
                        원가 복사
                      </button>
                    )}
                  </div>
                ...
                <div>
                  <label className={labelCls}>청구처 (고객)</label>
                  <AcInput
                    value={form.clientName}
                    onChange={v => setForm(f => ({ ...f, clientName: v, clientId: '' }))}
                    options={companyOptions}
                    placeholder="청구할 업체명 검색"
                    onSelect={opt => setForm(f => ({ ...f, clientName: opt.label, clientId: opt.id || '', companyId: opt.id || '' }))}
                  />
                </div>
                <div>
                  <label className={labelCls}>청구 통화</label>
                  <select className={inputCls + ' w-28'} value={form.billCurrency} onChange={e => setForm(f => ({ ...f, billCurrency: e.target.value }))}>
                    {CURRENCIES.map(c => <option key={c} value={c}>{c}</option>)}
                  </select>
                </div>
                ...
                {/* 이익 표시 */}
                {(costItemsTotal > 0 || parseFloat(form.costAmount || '0') > 0) && lineItemsTotal > 0 && (
                  <div className={cn(
                    'text-xs font-semibold px-3 py-2 rounded-lg',
                    margin > 0 ? 'bg-green-100 text-green-800' : margin < 0 ? 'bg-red-50 text-red-700' : 'bg-slate-100 text-slate-600'
                  )}>
                    {margin > 0 ? '잡이익' : margin < 0 ? '잡손실' : '손익 없음'}:&nbsp;
                    {margin >= 0 ? '+' : ''}{margin.toLocaleString(undefined, { maximumFractionDigits: 2 })} {form.billCurrency}
                    {effectiveCostAmount > 0 && (
                      <span className="ml-2 opacity-70">
                        ({((margin / effectiveCostAmount) * 100).toFixed(1)}%)
                      </span>
                    )}
                  </div>
                )}
              ...
              {!isNew && record && (
                <button type="button"
                  ...
                  className="flex items-center gap-1.5 text-xs text-orange-700 border border-orange-300 px-2.5 py-1.5 rounded-lg hover:bg-orange-100">
                  <Printer className="w-3.5 h-3.5" />인보이스/청구서 출력
                </button>
              )}
            </div>
          )}
