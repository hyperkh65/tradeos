...
import { Plus, Search, Boxes, X, Loader2 } from 'lucide-react';
...
function POModal({ onClose, onSave }: { onClose: () => void; onSave: (po: PurchaseOrder) => void }) {
  const [form, setForm] = useState({ supplierName: '', currency: 'USD', orderDate: new Date().toISOString().slice(0, 10), etd: '', paymentTerms: '30% T/T', incoterm: 'FOB', status: 'draft', productName: '', qty: '', unitPrice: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    const amount = Number(form.qty) * Number(form.unitPrice);
    const items = [{ productName: form.productName, qty: Number(form.qty), unitPrice: Number(form.unitPrice), amount }];
    try {
      const res = await fetch('/api/purchase-orders', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...form, items, totalAmount: amount }) });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-background rounded-xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-4 border-b sticky top-0 bg-background">
          <h2 className="font-semibold">새 발주</h2>
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <Input type="number" value={form.qty} onChange={e => setForm(f => ({ ...f, qty: e.target.value }))} placeholder="1000" />
            </div>
            <div>
              <label className="text-xs font-medium text-muted-foreground mb-1 block">단가</label>
              <Input type="number" value={form.unitPrice} onChange={e => setForm(f => ({ ...f, unitPrice: e.target.value }))} placeholder="17.50" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="text-xs font-medium text-muted-foreground mb-1 block">선적예정일</label>
          ...
          {form.qty && form.unitPrice && (
            <div className="bg-muted/50 rounded-lg p-3 text-sm">
              ...
              <span className="font-semibold">{form.currency} {(Number(form.qty) * Number(form.unitPrice)).toLocaleString()}</span>
            </div>
          )}
...
export default function PurchaseOrdersPage() {
  ...
  useEffect(() => {
    fetch('/api/purchase-orders').then(r => r.json()).then(j => { if (j.data) setPos(j.data); }).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <AppHeader title="발주" />
      {showModal && <POModal onClose={() => setShowModal(false)} onSave={po => { setPos(prev => [po, ...prev]); setShowModal(false); }} />}
      <div className="flex-1 overflow-y-auto p-4 md:p-5">
        <div className="flex flex-col sm:flex-row gap-2 mb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button size="sm" className="h-9 gap-1 shrink-0 ml-auto" onClick={() => setShowModal(true)}>
                ...
                <div key={po.id} className="bg-card border border-border rounded-xl p-4">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div className="bg-muted/50 rounded-lg p-2">
                      ...
                      <p className="font-semibold">{po.currency} {Number(po.totalAmount).toLocaleString()}</p>
                    </div>
                    <div className="bg-muted/50 rounded-lg p-2">
