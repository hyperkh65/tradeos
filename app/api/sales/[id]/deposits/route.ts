...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const db = getDb();
  const row = db.prepare('SELECT deposits_json, total_amount FROM sales WHERE id=?').get(id) as Record<string, unknown> | undefined;
  ...
  deposits.push(entry);
  db.prepare('UPDATE sales SET deposits_json=? WHERE id=?').run(JSON.stringify(deposits), id);

  const summary = summarizeDeposits((row.total_amount as number) || 0, deposits);
