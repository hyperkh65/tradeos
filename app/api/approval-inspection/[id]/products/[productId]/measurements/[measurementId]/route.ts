...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string; productId: string; measurementId: string }> }) {
  const user = await getSessionUser();
  if (!user) return NextResponse.json({ error: '인증이 필요합니다' }, { status: 401 });
  const { id, productId, measurementId } = await params;
  const db = getDb();
  ...
  const before = db.prepare('SELECT * FROM approval_inspection_measurements WHERE id=? AND product_id=? AND project_id=?').get(measurementId, productId, id) as Record<string, unknown> | undefined;
  ...
  writeInspectionAuditLog({ projectId: id, action: 'measurement_update', actorType: 'internal', actorUserId: user.id, actorUserName: user.name, before, req });
