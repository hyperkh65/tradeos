...
async function buildProductsForDoc(projectId: string, reportType: ReportType): Promise<DocProduct[]> {
  ...
  for (const p of products) {
    ...
    out.push({
      ...
      specText: (p.spec_text as string) || undefined,
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const meta: InspectionDocMeta = {
    ...
    productionQty: (project.production_qty as number) ?? undefined, inspectionQty: (project.inspection_qty as number) ?? undefined,
    defectQty: (project.defect_qty as number) ?? undefined,
