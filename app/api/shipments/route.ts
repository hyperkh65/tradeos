...
export async function GET(req: NextRequest) {
  ...
  const rows = db.prepare('SELECT * FROM shipments WHERE local_deleted=0 OR local_deleted IS NULL ORDER BY eta DESC NULLS LAST, created_at DESC').all() as Record<string, unknown>[];
