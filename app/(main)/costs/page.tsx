...
function CostModal({ record, onClose, onSave }: {
  ...
  const effectiveCostAmount = isDetailType && lineItems.length > 0 ? lineItemsTotal : parseFloat(form.costAmount || '0');
  const billFinal = parseFloat(form.billAmount || effectiveCostAmount.toString() || '0');
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const finalCostAmount = isDetailType && lineItems.length > 0 ? lineItemsTotal : parseFloat(form.costAmount || '0');
      const body = {
            ...
            <div className="bg-orange-50 border border-orange-200 rounded-lg p-3 space-y-3">
              <div className="flex items-center justify-between">
                <div className="text-xs font-semibold text-orange-800">
                  {form.costType === 'certification' ? '🏆 인증비 내역' : '🔧 A/S 현장 비용 내역'}
                </div>
                {lineItems.length > 0 && (
