...
interface SampleItem {
  ...
  chargeType: 'free' | 'paid'; amount: number; images: ItemImage[];
}
...
interface DocRow {
  ...
  data: SampleData; createdAt: string; updatedAt: string;
}
...
const emptyItem = (): SampleItem => ({ name: '', specification: '', qty: 0, unit: 'EA', remark: '', chargeType: 'free', amount: 0, images: [] });
const emptyData = (): SampleData => ({
  date: todayStr(), validUntil: addDays(30), currency: 'USD',
...
export default function SampleRequestPage() {
  ...
  const [data, setData] = useState<SampleData>(emptyData());
  ...
  const loadList = useCallback(() => {
    setLoadingList(true);
    fetch('/api/documents?type=sample_request').then(r => r.json()).then(j => setList(j.data || [])).finally(() => setLoadingList(false));
  }, []);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    const title = `${data.supplierName} 샘플의뢰서`;
    try {
      if (selected) {
        ...
        const r = await fetch('/api/documents', {
          ...
          body: JSON.stringify({ docType: 'sample_request', title, data, status: 'issued' }),
        });
        const j = await r.json();
        if (!r.ok) { alert(j.error || '저장 실패'); return; }
        setSelected(j.data);
  ...
  const updateItem = (idx: number, patch: Partial<SampleItem>) => {
  ...
  // 기존에 등록된 제품을 선택하면 규격/제품사진을 그대로 가져와 채워준다
  ...
  const paidTotal = data.items.reduce((s, it) => s + (it.chargeType === 'paid' ? (it.amount || 0) : 0), 0);
    ...
    <div className="flex flex-col h-full overflow-hidden bg-muted/40">
      ...
      <div className="flex items-center gap-2 px-4 h-14 bg-slate-900 text-white shrink-0">
        <span className="font-semibold text-sm mr-auto truncate">
          샘플 의뢰서 Sample Request {selected ? `· ${selected.businessId}` : '(신규 New)'}
        </span>
        <Button size="sm" variant="outline" className="h-8 text-xs gap-1 bg-transparent border-white/25 text-white hover:bg-white/10 hover:text-white" onClick={openNew}>
          <FilePlus2 className="w-3.5 h-3.5" />새 샘플의뢰서
        </Button>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <div className="max-w-4xl mx-auto bg-white rounded-xl shadow-sm border border-border p-10">
          ...
          <div className="flex items-start justify-between gap-6 mb-2">
            <div>
              ...
              <div className="text-xs text-muted-foreground space-y-1 mt-3">
                ...
                <div>사업자번호 Biz No: {company?.bizNo}</div>
              </div>
            </div>
            <div className="text-right shrink-0">
              <div className="text-2xl font-bold text-emerald-700">샘플 의뢰서</div>
              <div className="text-[10px] text-muted-foreground tracking-widest mb-3">SAMPLE REQUEST FORM</div>
              <div className="space-y-1.5">
                <div className="flex items-center justify-end gap-3">
                  <span className="text-[11px] text-muted-foreground">문서번호 No.</span>
                  <span className="text-sm font-bold">{selected?.businessId || '(저장 시 자동부여)'}</span>
                </div>
                <div className="flex items-center justify-end gap-3">
                  <span className="text-[11px] text-muted-foreground">작성일 Date</span>
                ...
                <div className="flex items-center justify-end gap-3">
                  <span className="text-[11px] text-muted-foreground">유효기한 Valid Until</span>
          ...
          <div className="h-0.5 bg-emerald-600 my-6" />

          {/* FROM / TO */}
          <div className="grid grid-cols-2 gap-6 mb-8">
            <div className="bg-emerald-50 rounded-lg p-4">
              <div className="text-[11px] font-bold text-emerald-700 tracking-wide mb-2">구매자 FROM (BUYER)</div>
              ...
              <div className="text-xs text-muted-foreground">사업자번호 Biz No: {company?.bizNo}</div>
            </div>
            <div className="border border-border rounded-lg p-4">
              <div className="text-[11px] font-bold text-muted-foreground tracking-wide mb-3">공급사 TO (SUPPLIER)</div>
              <div className="space-y-2.5">
                <div>
                  <label className="text-[10px] font-medium text-muted-foreground tracking-wide">회사명 COMPANY</label>
                  <input list="sr-suppliers" value={data.supplierName} onChange={e => applySupplier(e.target.value)}
              ...
              <div key={i} className="border border-border rounded-lg overflow-hidden">
                <div className="flex items-center justify-between bg-slate-900 text-white px-3 py-1.5">
                  ...
                  <button type="button" onClick={() => removeItem(i)} className="text-[11px] text-red-300 hover:text-red-100 flex items-center gap-1">
                    <X className="w-3 h-3" />삭제 Remove
                  </button>
                </div>
                <div className="grid grid-cols-6 gap-3 p-3">
                  <div className="col-span-2">
                    <label className="text-[10px] text-muted-foreground">품목명 ITEM (등록된 제품 선택 시 규격·사진 자동입력)</label>
                    <input list="sr-products" value={it.name} onChange={e => applyProduct(i, e.target.value)} placeholder="예: LED 모듈 25W 주광색"
                  ...
                  <div>
                    <label className="text-[10px] text-muted-foreground">규격 SPEC</label>
                    <input value={it.specification} onChange={e => updateItem(i, { specification: e.target.value })}
                      className="w-full text-xs px-2 py-1.5 rounded border border-input bg-background" />
                  </div>
                  <div>
                    <label className="text-[10px] text-muted-foreground">단위 UNIT</label>
                    <input value={it.unit} onChange={e => updateItem(i, { unit: e.target.value })}
                      className="w-full text-xs px-2 py-1.5 rounded border border-input bg-background" />
                  </div>
                  <div>
                    <label className="text-[10px] text-muted-foreground">수량 QTY</label>
                    <input type="number" value={it.qty || ''} onChange={e => updateItem(i, { qty: Number(e.target.value) || 0 })}
                      className="w-full text-xs px-2 py-1.5 rounded border border-input bg-background" />
                  </div>
                  <div>
                    <label className="text-[10px] text-muted-foreground">구분 TYPE</label>
                    <select value={it.chargeType} onChange={e => updateItem(i, { chargeType: e.target.value as 'free' | 'paid' })}
                      className="w-full text-xs px-2 py-1.5 rounded border border-input bg-background">
                      <option value="free">무상 Free</option>
                      <option value="paid">유상 Paid</option>
                    </select>
                  </div>
                  <div>
                    <label className="text-[10px] text-muted-foreground">금액 AMOUNT ({data.currency})</label>
                    <input type="number" value={it.amount || ''} disabled={it.chargeType === 'free'}
                      onChange={e => updateItem(i, { amount: Number(e.target.value) || 0 })}
                      className="w-full text-xs px-2 py-1.5 rounded border border-input bg-background disabled:opacity-40 disabled:bg-muted" />
                  </div>
                  <div className="col-span-3">
                    <label className="text-[10px] text-muted-foreground">비고 REMARK</label>
                    <input value={it.remark} onChange={e => updateItem(i, { remark: e.target.value })}
                      className="w-full text-xs px-2 py-1.5 rounded border border-input bg-background" />
                  </div>
                  <div className="col-span-3">
                    <label className="text-[10px] text-muted-foreground block mb-1">제품 사진 PHOTO</label>
          ...
          {paidTotal > 0 && (
            <div className="flex justify-end mt-3">
              <div className="bg-emerald-50 rounded-lg px-4 py-2 text-sm">
                <span className="text-muted-foreground mr-2">유상품목 합계 Paid Total</span>
                <span className="font-bold text-emerald-700">
                  {paidTotal.toLocaleString(data.currency === 'KRW' ? 'ko-KR' : 'en-US', { maximumFractionDigits: 2 })} {data.currency}
                </span>
              </div>
            </div>
          )}
          ...
          {/* Remark */}
          <div className="mt-6">
            <label className="text-xs font-semibold mb-1 block">요청사항 NOTE</label>
            <textarea className="w-full min-h-[80px] text-sm rounded-md border border-input bg-background px-3 py-2"
              ...
              placeholder="예: 샘플 수령 후 회신 예정일, 배송 방법 등 요청사항을 입력하세요." />
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setListOpen(false)}>
          <div className="bg-background rounded-xl shadow-2xl w-[480px] max-h-[75vh] flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-4 py-3 border-b border-border">
              <span className="text-sm font-semibold">샘플 의뢰서 목록</span>
            ...
            <div className="flex-1 overflow-y-auto">
              {loadingList ? (
                ...
                <div className="text-center text-sm text-muted-foreground py-12">작성된 샘플의뢰서가 없습니다</div>
              ) : filteredList.map(d => (
                <div key={d.id}
                  className={cn('px-4 py-2.5 border-b border-border cursor-pointer hover:bg-muted/40 flex items-center gap-2', selected?.id === d.id && 'bg-emerald-50')}
                  onClick={() => openDoc(d)}>
