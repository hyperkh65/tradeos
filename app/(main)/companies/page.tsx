...
import { Building2, Globe, Phone, Mail, Plus, Search, X, Loader2 } from 'lucide-react';
...
function CompanyModal({ onClose, onSave }: { onClose: () => void; onSave: (c: Company) => void }) {
  const [form, setForm] = useState({ name: '', nameEn: '', type: '공급업체', country: '중국', phone: '', email: '', contactPerson: '' });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-background rounded-xl shadow-2xl w-full max-w-md">
        <div className="flex items-center justify-between p-4 border-b">
          <h2 className="font-semibold">거래처 추가</h2>
...
export default function CompaniesPage() {
  ...
  useEffect(() => {
    fetch('/api/companies').then(r => r.json()).then(j => { if (j.data) setCompanies(j.data); }).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <AppHeader title="거래처" />
      {showModal && <CompanyModal onClose={() => setShowModal(false)} onSave={c => { setCompanies(prev => [c, ...prev]); setShowModal(false); }} />}
