...
function computeBillStatus(
  ...
  switch (disposition) {
    ...
    case 'billable_domestic':
      ...
      return 'invoiced';
    case 'selling_admin':
      return linkedSales.length > 0 ? 'collected' : 'unbilled';
...
const BILL_STATUS_DISPLAY: Record<string, Record<string, { label: string; color: string }>> = {
  ...
  selling_admin: {
    unbilled:  { label: '매출연결 필요', color: 'text-amber-600' },
    collected: { label: '완료', color: 'text-green-600' },
  },
  billable_foreign: {
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          {form.disposition === 'selling_admin' && (
            <div className="bg-orange-50 border border-orange-200 rounded-lg p-3 space-y-3">
              <div className="flex items-center justify-between">
                <div className="text-xs font-semibold text-orange-800">판매관리비 — 매출 연결</div>
                {(() => {
                  const autoStatus = computeBillStatus('selling_admin', linkedSales, 0, null, form.billStatus, [], effectiveCostAmount);
              ...
              {/* 2열: 연결된 매출(좌) | 매출 목록 선택(우) */}
              <div className="grid grid-cols-2 gap-3">
                {/* 좌: 연결된 매출 */}
                <div>
                  <div className="text-[11px] font-medium text-orange-700 mb-1.5">연결된 매출 ({linkedSales.length}건)</div>
                  {linkedSales.length === 0 ? (
                    <div className="text-xs text-muted-foreground py-6 text-center border border-dashed border-orange-200 rounded-lg bg-white">
                      오른쪽 목록에서 매출을 선택하세요
                    </div>
                  ) : (
                    <div className="space-y-1.5 max-h-64 overflow-y-auto">
                      {linkedSales.map(ls => {
                        const sale = ref.sales.find(s => s.id === ls.id);
                        return (
                          <div key={ls.id} className="flex items-center justify-between bg-white border border-orange-200 rounded-lg px-2.5 py-2 text-xs">
                            <div className="min-w-0 flex-1">
                              <div className="font-semibold text-[11px]">{ls.businessId}</div>
                              {sale && (
                                <div className="text-[10px] text-muted-foreground">
                                  {sale.customer || '—'} · {sale.saleDate || ''}<br />
                                  <span className="font-medium text-foreground">{sale.totalAmount?.toLocaleString() || '—'}원</span>
                                </div>
                              )}
                            </div>
                            <button type="button"
                              onClick={() => setLinkedSales(prev => prev.filter(s => s.id !== ls.id))}
                              className="ml-2 shrink-0 text-muted-foreground hover:text-red-600 transition-colors">
                              <X className="w-3.5 h-3.5" />
                            </button>
                          </div>
                        );
                ...
                {/* 우: 매출 목록 */}
                <div>
                  <div className="text-[11px] font-medium text-orange-700 mb-1.5">매출 목록</div>
                  <LinkedSalesSelector
                    linkedSales={linkedSales}
                    onChange={setLinkedSales}
                    sales={ref.sales}
