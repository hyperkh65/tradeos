...
function dbToClaim(row: Record<string, unknown>) {
  return {
...
export async function POST(req: NextRequest) {
  try {
    ...
    db.prepare(`INSERT INTO claims (id,business_id,customer_id,customer_name,supplier_id,supplier_name,product_id,product_name,po_id,po_business_id,issue_type,description,claim_amount,currency,compensation_type,compensation_amount,status,created_by,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`)
      .run(id, bizId, body.customerId ?? null, body.customerName ?? null, body.supplierId ?? null, body.supplierName ?? null, body.productId ?? null, body.productName ?? null, body.poId ?? null, body.poBusinessId ?? null, body.issueType, body.description, body.claimAmount ?? null, body.currency ?? null, body.compensationType ?? null, body.compensationAmount ?? null, body.status || '접수', 'user-1', ts, ts);
