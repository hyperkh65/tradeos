...
export async function POST(req: NextRequest) {
  ...
  const id = newId();
  const ts = now();
  db.prepare(`INSERT INTO estimator_cases (id,name,container_type,freight_sea,freight_inland,freight_port,freight_misc,fx_usd,fx_rmb,duty_rate,sim_mode,items_json,notes,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`)
    .run(id, body.name || '새 케이스', body.containerType || '40ft',
      body.freightSea ?? 930000, body.freightInland ?? 250000, body.freightPort ?? 280000, body.freightMisc ?? 0,
      body.fxUsd ?? 1330, body.fxRmb ?? 185, body.dutyRate ?? 0.024,
